 ...
 */
if ( ! class_exists( 'Iva_Business_Hours_Pro' ) ) {
	class Iva_Business_Hours_Pro {
		// Constructor
		function __construct() {
			add_action( 'init', array( $this, 'iva_business_hours_pro_init' ) );
			add_shortcode( 'iva_bhrs_pro', array( $this, 'iva_business_hours_pro_shortcode' ) );
		}

		// Initialize the location function
		function iva_business_hours_pro_init() {
			if ( function_exists( 'vc_map' ) ) {
				global $wpdb;
				$ivbh_select_query  = "SELECT * FROM $wpdb->iva_businesshours" ;
				$ivbh_fetch_results = $wpdb->get_results( $ivbh_select_query );
				$iva_options = array( 'Select Hours' => '' );
				...
				vc_map(
					array(
					   'name' 			=> __( 'Business Hours Pro', 'iva_business_hours' ),
					   'base' 			=> 'iva_bhrs_pro',
					   'class' 			=> '',
					   'icon' 			=> '',
					   ...
					   'params' 		=> array(
							...
							array(
								'type' 			=> 'checkbox',
								'class' 		=> '',
								'heading' 		=> __( 'Shortcode Title:', 'iva_business_hours' ),
								'param_name' 	=> 'title',
								'value'			=> array( 'Yes' => 'true' ),
								'description' 	=> __( 'Check this if you wish to display the title.', 'iva_business_hours' ),
							),
							array(
								'type' 			=> 'checkbox',
								'class' 		=> '',
							...
							array(
								'type'			 => 'textfield',
								'class' 		 => '',
								'heading' 		 => __( 'Custom Width', 'iva_business_hours' ),
								'param_name' 	 => 'width',
								'value' 		 => '',
		...
		function iva_business_hours_pro_shortcode( $atts ) {
			...
			$out = $output = $today_output = $iva_bh_single_day = $toggle_output = $iva_bh_style = $iva_algncenter_hrs = $iva_bh_algn_class = $holiday_closed = $holiday_name = $iva_holidays_date_text = $iva_custom_width = $iva_oc_title = '';//stores the output
		...
		if ( $iva_bh_results ) {
			...
			foreach ( $iva_bh_results as $iva_bh_data ) {
				...
				$iva_toggle_enable = $closed_css = $iva_oc_css = $iva_oc_class = $iva_open_image = $iva_close_image = $iva_oc_img = $today_color = $iva_bh_start_hrs = $iva_bh_end_hrs = $iva_oc_text = $iva_singleday_disable = $iva_days_shortname = $holiday_closed = $holiday_name = $holidays_extras = '';
				$iva_oc_title = $iva_open_title = $iva_close_title = $iva_disable_dots ='';
				...
				if ( isset( $days_shortname ) && '' != $days_shortname ) {
					$iva_days_shortname = $days_shortname;
				}
				...
				if ( isset( $custom_width ) && '' != $custom_width ) {
					$iva_custom_width = $custom_width;
				}
				...
				if ( isset( $disable_dots ) && !empty( $disable_dots ) ) {
					$iva_disable_dots = $disable_dots;
				}
				...
				if ( 'on' == $iva_latenight_hrs_enable ) {
					if ( ( strtotime( $iva_curent_time ) >= strtotime( '0:00' ) &&
					strtotime( $iva_curent_time ) <= strtotime( $iva_latenight_hrs ) ) ) {
						...
						foreach ( $value as $time ) {
							...
							if ( ( 'on' == $iva_toggle_enable || 'on' == $iva_singleday_show || isset( $iva_oc_text_hide ) ) && 'on' != $iva_singleday_disable ) {
								...
								if ( $iva_bh_today == $day ) {
										...
										if ( ( '' == $ivbh_day_open ) && ( '' == $ivbh_day_close ) && ( '' == $ivbh_day_end ) && ( '' == $ivbh_day_start ) ) {
											$today_output .= '<span class="hours-row"><span class="closed" ' . $closed_css . '>' . $iva_closed_text . '</span></span></div>';
										} else {
		...
		// Result
		$output .= '<div class="iva_bh_shortcode ' . $iva_bh_single_day . ' ' . $iva_bh_algn_class . ' " ' . $custom_width_css . '>';
		$output .= $today_output;
...
if ( class_exists( 'Iva_Business_Hours_Pro' ) ) {
	$iva_business_hours_pro = new Iva_Business_Hours_Pro;
}
